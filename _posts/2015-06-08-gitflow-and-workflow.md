...
The first thing we need is a repository to work on. This will already be created on GitHub under the Praekelt or Unicore Organisations. You will the clone the repo using
...
You'll get a bunch of options that you need to confirm that looks liks this:
{% highlight c %}
...
Then you'll need to install the packages that will allow the code to run locally on your machine. The 
...
This is the standard way to set things up, but it may differ from repo to repo. Check out the local readme file for instructions. There may also be a `bootstrap.sh` file in the repo, in which case you can skip the intructions from the virtualenv to here and simply run
...
Great! So your local machine *should* be set up, but there are invaraibly problems the first couple of times. Ask a real live human to help you out :)
...
You've just created a branch on your local machine, but you now need to show everyone what you're doing by gettin that branch on to the GitHub servers. You do this using the following:
...
Now, you need to update the GiHub repo to ensure that if any wants to look at your code, it's up to date. You do that using:
...
###Conclusion
There are some things that were not covered in depth here like testing and some of the tools we use, but this has been a breakdown and explanation of the workflow and how we use git, github and gitflow to maximise our code quality and make everyone's jobs easier, although it might not seem very simple right now!

Things to note:

- Things like quality assurance from project managers and the broader software creation process is not covered here. 
- I [have not yet created an article that summarises the above commands](brokenlink).
- Here's [an article that just about gitflow](http://danielkummer.github.io/git-flow-cheatsheet/) that may explain things in a more general sense.